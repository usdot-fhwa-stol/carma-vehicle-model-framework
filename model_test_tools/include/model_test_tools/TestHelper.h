...
namespace model_test_tools {
  ...
  class SteeringTestHelper {
    public: 
      ...
      const double timestep_ = 0.05;

      SteeringTestHelper(bool write_to_file, double timestep) : write_to_file_(write_to_file) {}
      ...
      void steeringCallback (
        ...
        setpoints.push_back(target_steer);
        current_steers.push_back(current_steer);
      ...
      std::tuple<double, double> evaluateSteeringData(lib_vehicle_model::VehicleMotionModel& pcm) {
        ...
        for (size_t n = 0; n < setpoints.size(); n++) {
          ...
          if (firstLoop) {
            ...
            std::vector<double> subForcast = predictSteerFull(pcm, initialSteer, prevSetpoint, true, timestep_, subCount);
        ...
        return model_test_tools::getMaxErrorAndRMSE(forcasts,current_steers);
      }
  };
